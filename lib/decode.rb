  ...
  include Shiftable
    ...
    def decrypt_a_key(count, character)
    ...
    def decrypt_b_key(count, character)
      if @alphabet.include?(character)
        original_index = @alphabet.find_index(character)
    ...
    def decrypt_c_key(count, character)
      if @alphabet.include?(character)
        original_index = @alphabet.find_index(character)
    ...
    def decrypt_d_key(count, character)
      if @alphabet.include?(character)
        original_index = @alphabet.find_index(character)
    ...
    def decode_message
      ...
      @message.each do |character|
          if count == 0 || count % 4 == 0
            @decrypted_message << decrypt_a_key(count, character)
          elsif count == 1 || count % 4 == 1
            @decrypted_message << decrypt_b_key(count, character)
          elsif count == 2 || count % 4 == 2
            @decrypted_message << decrypt_c_key(count, character)
          elsif count == 3 || count % 4 == 3
            @decrypted_message << decrypt_d_key(count, character)
          end
          count += 1
          @decrypted_message
        end
